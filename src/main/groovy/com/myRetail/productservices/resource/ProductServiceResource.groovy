...
class ProductServiceResource {
    ...
    ResponseEntity getProductDetails (@ApiParam(value = '123456', required = true) @PathVariable("id") Integer id){
        ...
        try {
           ...
           ProductDetailsResponse response = new ProductDetailsResponse(id: id, message: "Product not found in redsky")
        ...
        if (productDetailsResponse.current_price?.value == null){
            ProductDetailsResponse response = new ProductDetailsResponse(id: id, message: "Product does not have a price")
            return new ResponseEntity(objectMapper.writeValueAsString(response),HttpStatus.NOT_FOUND)
        }
        return new ResponseEntity(objectMapper.writeValueAsString(productDetailsResponse),HttpStatus.OK)
